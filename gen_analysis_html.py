...
function renderShareholders() {
  ...
  if (!sh || !sh.shareholders || Object.keys(sh.shareholders).length === 0) {
...
function _renderShTable(sh, catFilter, dirFilter, quarter) {
  ...
  for (const cat of catOrder) {
    ...
    for (const item of items) {
      ...
      h += '<td>' + item.holder.name + '</td>';
      h += '<td>' + (item.holder.holdNum ? (item.holder.holdNum / 10000).toFixed(0) + '万' : '-') + '</td>';
      h += '<td>' + (item.holder.holdRatio ? item.holder.holdRatio.toFixed(2) + '%' : '-') + '</td>';
      h += '<td><span class="dir-badge ' + dirCls + '">' + (item.holder.direction || '-') + '</span></td>';
      h += '<td>第' + item.holder.rank + '</td>';
      h += '</tr>';
    }
    h += '</tbody></table></div>';
  }
