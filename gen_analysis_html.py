...
# 读取QDII数据
QDII_FILE = os.path.join(os.path.dirname(CSV_FILE), "qdii.json")
qdii_data = {}
if os.path.exists(QDII_FILE):
    with open(QDII_FILE, 'r', encoding='utf-8') as f:
        qdii_data = json.load(f)
...
<div class="nav-tabs">
  ...
  <div class="nav-tab" onclick="switchTab('tab-shareholders')">📋 股东追踪</div>
  <div class="nav-tab" onclick="switchTab('tab-qdii')">🌐 QDII额度</div>
</div>
...
<div id="tab-summary" class="tab-content">
  ...
  <div id="qdiiPanel" style="display:block"></div>
...
<div id="tab-shareholders" class="tab-content">
  <div class="section-title">📋 十大流通股东追踪</div>
  <div class="section-sub">关注自选ETF持仓股的股东变化，帮你跟着社保、券商资金走</div>
  <div id="shareholderContent">
    <p style="color:#94a3b8;padding:40px;text-align:center">加载中...</p>
  </div>
</div>

<div id="tab-qdii" class="tab-content">
  <div class="section-title">🌐 合格境内机构投资者（QDII）投资额度</div>
  <div class="section-sub">国家外汇管理局每月发布的QDII额度审批情况</div>
  <div id="qdiiContent">
    <p style="color:#94a3b8;padding:40px;text-align:center">加载中...</p>
  </div>
</div>
...
function renderSummary(date, dd) {
  // ===== QDII额度提醒（页面顶部） =====
  const qdii = ALL_DATA._meta && ALL_DATA._meta.qdii;
  if (qdii && qdii.totalQuota) {
    ...
    qHtml += '<div style="font-size:12px;color:#475569;display:flex;gap:16px;flex-wrap:wrap">';
    qHtml += '<span>🏦 总计: <strong>' + qdii.totalQuota + '</strong>亿美元</span>';
    qHtml += '<span>🏛️ 机构: <strong>' + qdii.totalOrgs + '</strong>家</span>';
    qHtml += '<span>📅 最新批复: <strong>' + (qdii.latestApproveDate || '无') + '</strong></span>';
    qHtml += '<span>🔄 数据更新: <strong>' + qdii.reportDate + '</strong></span>';
    qHtml += '</div>';
    if (qdii.categories) {
      qHtml += '<div style="font-size:11px;color:#94a3b8;margin-top:4px">';
      for (const [cat, val] of Object.entries(qdii.categories)) {
        qHtml += '<span style="margin-right:12px">' + cat + ': ' + val + '亿</span>';
      }
      qHtml += '</div>';
    }
    if (isNew) {
    ...
    qHtml += '</div>';
    document.getElementById('qdiiPanel').innerHTML = qHtml;
  }
...
function renderShareholders() {
  ...
  const container = document.getElementById('shareholderContent');
  if (!sh || !sh.allEntries || sh.allEntries.length === 0) {
    container.innerHTML = '<p style="color:#94a3b8;padding:40px;text-align:center">暂无股东数据，请先运行 collect_shareholders.py</p>';
    return;
  }
...
function _renderShTable(sh, catFilter, dirFilter, startDate, endDate) {
  ...
  for (const cat of catOrder) {
    ...
    for (const item of filtered.slice(0, 30)) {
      const dirCls = item.direction === '增持' ? 'dir-in' : (item.direction === '减持' ? 'dir-out' : '');
      h += '<tr>';
      h += '<td><code>' + item.stockCode + '</code></td>';
      h += '<td>' + item.holderName + '</td>';
      h += '<td>' + (item.holdNum ? (item.holdNum / 10000).toFixed(0) + '万' : '-') + '</td>';
      h += '<td>' + (item.holdRatio ? item.holdRatio.toFixed(2) + '%' : '-') + '</td>';
      h += '<td><span class="dir-badge ' + dirCls + '">' + (item.direction || '-') + '</span></td>';
      h += '<td>第' + item.rank + '</td>';
      h += '</tr>';
    }
    h += '</tbody></table></div>';
...
function renderQDII() {
  const meta = ALL_DATA._meta;
  const qdii = meta && meta.qdii;
  const container = document.getElementById('qdiiContent');
  if (!qdii || !qdii.totalQuota) {
    container.innerHTML = '<p style="color:#94a3b8;padding:40px;text-align:center">暂无QDII数据，请先运行 collect_qdii.py</p>';
    return;
  }
  ...
  h += '<div class="stat-card"><div class="num" style="color:#2563eb">' + qdii.totalQuota + '</div><div class="lbl">总计(亿美元)</div></div>';
  h += '<div class="stat-card"><div class="num">' + qdii.totalOrgs + '</div><div class="lbl">机构总数</div></div>';
  h += '<div class="stat-card"><div class="num" style="color:#16a34a">' + (qdii.reportDate || '-') + '</div><div class="lbl">报告期</div></div>';
  h += '<div class="stat-card"><div class="num" style="color:#f59e0b">' + (qdii.latestApproveDate || '-') + '</div><div class="lbl">最新批复</div></div>';
  h += '</div>';

  // 分类统计
  if (qdii.categories) {
    h += '<h4 style="margin:12px 0 8px">📊 分类统计</h4>';
    h += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-bottom:16px">';
    for (const [cat, val] of Object.entries(qdii.categories)) {
      const pct = qdii.totalQuota > 0 ? (val / qdii.totalQuota * 100).toFixed(1) : 0;
      h += '<div style="padding:10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px">';
      h += '<div style="font-size:11px;color:#64748b">' + cat + '</div>';
      h += '<div style="font-size:16px;font-weight:600;color:#1e293b">' + val + '<span style="font-size:11px;color:#94a3b8;font-weight:400"> 亿</span></div>';
      h += '<div style="font-size:11px;color:#94a3b8">占比' + pct + '%</div>';
      h += '</div>';
    }
    h += '</div>';
  }
  ...
  const newOrgs = qdii.newOrgs || [];
  const quotaUp = qdii.quotaIncrease || [];
  if (newOrgs.length > 0 || quotaUp.length > 0) {
    h += '<div class="sell-alert" style="border-left-color:#2563eb"><h3 style="color:#2563eb">🚀 最近更新</h3>';
    if (newOrgs.length > 0) {
      h += '<h4 style="margin:8px 0;font-size:13px">✨ 新晋机构 (' + newOrgs.length + '家)</h4>';
      h += '<div class="backtest-box"><table style="font-size:12px"><thead><tr><th>机构名称</th><th>批准日期</th><th>额度(亿美元)</th></tr></thead><tbody>';
      for (const o of newOrgs) {
        h += '<tr><td>' + o.name + '</td><td>' + o.approveDate + '</td><td style="font-weight:600;color:#2563eb">' + o.quota + '</td></tr>';
      }
      h += '</tbody></table></div>';
    }
    if (quotaUp.length > 0) {
      h += '<h4 style="margin:8px 0;font-size:13px">📈 额度提升 (' + quotaUp.length + '家)</h4>';
      h += '<div class="backtest-box"><table style="font-size:12px"><thead><tr><th>机构名称</th><th>原额度</th><th>新额度</th><th>增加</th></tr></thead><tbody>';
      for (const q of quotaUp) {
        h += '<tr><td>' + q.name + '</td><td>' + q.oldQuota + '亿</td><td>' + q.newQuota + '亿</td><td style="font-weight:600;color:#16a34a">+' + q.increase + '亿</td></tr>';
      }
      h += '</tbody></table></div>';
    }
    h += '</div>';
  ...
  // 全部机构列表（可搜索）
  h += '<h4 style="margin:16px 0 8px">🏛️ 全部机构清单 <span class="count-badge">' + qdii.totalOrgs + '家</span></h4>';
  h += '<input type="text" id="qdiiSearch" placeholder="搜索机构名称..." oninput="filterQDII()" style="padding:6px 10px;border:1px solid #cbd5e1;border-radius:6px;font-size:13px;width:100%;max-width:400px;margin-bottom:10px">';
  h += '<div class="table-wrap"><table style="font-size:12px" id="qdiiTable"><thead><tr>';
  h += '<th>序号</th><th>机构名称</th><th>最新批准日期</th><th>累计批准额度(亿美元)</th></tr></thead><tbody id="qdiiTbody">';

  const records = qdii.allRecords || [];
  // 按额度降序
  records.sort((a, b) => b.quota - a.quota);
  for (const r of records) {
    h += '<tr class="qdii-row"><td>' + r.rank + '</td><td class="name-cell">' + r.name + '</td><td>' + r.approveDate + '</td><td style="font-weight:600">' + r.quota + '</td></tr>';
  }
  h += '</tbody></table></div>';

  h += '<div class="footnote" style="margin-top:16px">📅 数据来源: 国家外汇管理局 · ' + (qdii.reportDate || '') + ' · 更新于 ' + (qdii.fetchedAt || '') + '</div>';
  container.innerHTML = h;
}

// QDII搜索过滤
function filterQDII() {
  const q = document.getElementById('qdiiSearch').value.toLowerCase();
  const rows = document.querySelectorAll('#qdiiTbody tr');
